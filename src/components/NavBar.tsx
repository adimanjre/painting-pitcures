...
const NavBar: React.FC = () => {
  ...
  const isTrue = useMatchMedia("screen and (max-width:726px)");
  ...
  return (
    <div className="relative w-full">
      <nav className="flex inset-x-0 z-50 fixed w-full max-w-9/10 top-5 justify-between items-center text-white bg-black border border-[#747178] rounded-lg p-2 mx-auto">
        <div className="logo">
          <img className="h-15 w-full object-contain" src="logo.png" />
        </div>
        ...
        </button>
        {
          <ul 
          ref={menu}
          className="hidden md:flex items-center gap-5 flex-col md:flex-row absolute md:relative bg-black md:bg-transparent rounded-2xl md:rounded-none p-5 md:p-0 top-[110%] w-full md:w-auto left-0 border border-[#747178] md:border-0">
        ...
        <div className="btn-container hidden md:block">
          <Button classes="bg-[#ffffff] text-black flex items-center gap-3">
            <span>Get Started</span>
            <ArrowIcon color="#DC1D13" />
          </Button>
